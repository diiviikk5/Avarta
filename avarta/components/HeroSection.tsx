...
const NAV_LINKS = [
  { label: "4D Corridor", href: "/dashboard" },
  { label: "Downscaler", href: "/dashboard" },
  { label: "Physics Guard", href: "/dashboard" },
  { label: "Live Telemetry", href: "/api/threats" },
];
...
export default function HeroSection() {
  ...
  return (
    <section className="relative w-full min-h-screen bg-black select-none flex flex-col justify-between overflow-hidden">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at center, rgba(0,0,0,0.45) 0%, rgba(0,0,0,0.3) 50%, rgba(0,0,0,0.75) 100%), linear-gradient(to bottom, rgba(0,0,0,0.4) 0%, rgba(0,0,0,0.15) 35%, rgba(0,0,0,0.2) 65%, rgba(0,0,0,0.7) 100%)",
          zIndex: 1,
        }}
      />
        ...
        >
          ...
          >
            ...
            <Link
              href="/dashboard"
              onClick={() => setMenuOpen(false)}
              className="mt-4 px-8 py-3.5 rounded-full bg-white text-black text-base font-semibold hover:bg-neutral-200 transition-colors shadow-lg"
              style={{
                fontFamily: "system-ui, sans-serif",
              }}
            >
      ...
      >
        ...
        <nav className="flex items-center justify-between w-full">
          ...
          <Link
            href="/"
            className="text-white text-2xl sm:text-3xl select-none tracking-tight drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)] hover:opacity-90 font-serif"
          ...
          {/* Desktop nav pill */}
          <div className="hidden md:flex liquid-glass items-center gap-1 rounded-full px-3 py-1.5 bg-black/40 backdrop-blur-md border border-white/15 shadow-lg">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.label}
                href={link.href}
            ...
            <Link
              href="/dashboard"
              className="ml-2 px-5 py-1.5 rounded-full bg-white text-black text-sm font-semibold hover:bg-neutral-200 transition-colors shadow-sm"
              style={{ fontFamily: "system-ui, sans-serif" }}
            >
              Launch Console
            </Link>
        ...
        <div className="flex flex-col items-center justify-center flex-1 text-center gap-5 sm:gap-6 -mt-3 sm:mt-0 max-w-4xl mx-auto">
          {/* Badge */}
          <div className="liquid-glass rounded-full px-4 py-2 flex items-center gap-2 bg-black/40 backdrop-blur-md border border-white/15 shadow-xl">
            <span className="w-2.5 h-2.5 rounded-full bg-blue-400 radar-pulse inline-block shrink-0 shadow-[0_0_8px_rgba(96,165,250,0.8)]" />
            <span
          ...
          {/* Single Clean Enter Console Button (No AI Slop email form) */}
          <div className="mt-2">
            <Link
              href="/dashboard"
              className="px-8 py-3.5 rounded-full bg-white text-black text-sm font-semibold tracking-wide hover:bg-neutral-200 transition-all duration-300 shadow-[0_4px_24px_rgba(255,255,255,0.25)] hover:scale-105 inline-flex items-center gap-2 cursor-pointer"
              style={{ fontFamily: "system-ui, sans-serif" }}
            >
          ...
          {/* Video switcher */}
          <div className="flex items-center gap-2 sm:gap-4 mt-3 bg-black/40 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-white/15 shadow-xl">
            {VIDEOS.map((v, i) => (
              <button
                key={v.label}
        ...
        >
          {STATS.map((s, i) => (
            <span
              key={s}
              className="flex items-center gap-4 text-xs sm:text-sm text-white font-medium drop-shadow-[0_1px_4px_rgba(0,0,0,0.85)]"
            >
